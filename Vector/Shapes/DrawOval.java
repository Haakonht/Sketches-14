...
public class DrawOval extends Shapes {
	...
	private String description = "Oval";
	private int rotation;
	
	public DrawOval(Color outline, Color fill, int thickness, Point start, Point end) {
		outlineColor = outline;
		fillColor = fill;
		outlineThickness = thickness;
		this.start = start;
		this.end = end;
		shape = drawOval(start, end);
		selected = false;
		rotation = 0;
	}
	
	//SHAPE
	public static Ellipse2D.Float drawOval(Point start, Point end) {
	...
	public Graphics2D drawShape(Graphics g) {
		...
		graphSettings.fill(drawOval(start, end));	
		graphSettings.setColor(outlineColor);
		graphSettings.draw(drawOval(start, end));
